...
pyinstaller --noconfirm --clean --onefile --windowed --paths . --add-data "transition_effect_pack.zip;." --hidden-import cli --hidden-import project_loader --hidden-import media_index --hidden-import duration_probe --hidden-import timeline_sync --hidden-import project_writer --hidden-import transition_tools --hidden-import builtin_transition_catalog --hidden-import keyframe_tools --name capcut_gui_v3.9.6 gui.py

if not exist D:\capcut_adapter_test mkdir D:\capcut_adapter_test
copy /Y "%cd%\dist\capcut_gui_v3.9.6.exe" "D:\capcut_adapter_test\capcut_gui_v3.9.6.exe" >nul

echo.
echo Build done. EXE path:
echo D:\capcut_adapter_test\capcut_gui_v3.9.6.exe
