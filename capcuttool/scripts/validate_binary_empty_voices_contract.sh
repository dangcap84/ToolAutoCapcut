...
WORKDIR="${2:-$(mktemp -d)}"
mkdir -p "$WORKDIR"
OUTDIR="$WORKDIR/empty_voices_contract"
mkdir -p "$OUTDIR"
...
python3 - <<'PY' "$IMAGES_DIR/scene_001.png"
import sys
...
set +e
"$BIN_PATH" --mode inspect --project "$PROJECT_DIR" --images "$IMAGES_DIR" --voices "$VOICES_DIR" > "$OUTDIR/empty_voices_stdout.txt" 2> "$OUTDIR/empty_voices_stderr.txt"
EXIT_CODE=$?
set -e
printf '%s\n' "$EXIT_CODE" > "$OUTDIR/empty_voices_exit_code.txt"

if [[ "$EXIT_CODE" -eq 0 ]]; then
  echo "EMPTY_VOICES_CONTRACT_FAIL: expected non-zero exit code" > "$OUTDIR/empty_voices_check.txt"
  exit 4
fi

if ! grep -Fq "No voice files found in:" "$OUTDIR/empty_voices_stderr.txt"; then
  echo "EMPTY_VOICES_CONTRACT_FAIL: missing expected error text" > "$OUTDIR/empty_voices_check.txt"
  exit 5
fi

echo "EMPTY_VOICES_CONTRACT_OK" > "$OUTDIR/empty_voices_check.txt"
